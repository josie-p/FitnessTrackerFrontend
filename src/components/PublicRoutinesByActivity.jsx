    ...
    <div id="publicRoutinesByActivityPage">
      ...
      {publicRoutines?.length ? (
        publicRoutines.map((routine, idx) => {
          return (
            <div
              className="public-routine-cards"
              key={`the unique index for this is ${idx}`}
            >
              <h4 className="publicRoutinesCreatorName">
                <span className="beforeContent">from: </span>
                {routine.creatorName}
              </h4>
              ...
              {routine.activities?.length
                ? routine.activities.map((activity, idx) => {
                    return (
                      <div
                        key={`the unique index for this activity is ${idx}`}
                        className="activity-card-onPublicRoutine"
                      >
                        <p className="publicRoutinesActivityName">
                          {activity.name}
                        </p>
                        <p>
