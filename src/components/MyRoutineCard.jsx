...
const myRoutineCard = (props) => {
  ...
  const deleteActivity = async (routineActivityId, token) => {
    if (confirm("are you sure?") === true) {
      ...
      if (!response?.id) {
        setMessage("Delete failed complete- try again later!");
      }
  ...
  const deleteRoutine = async (routineId, token) => {
    if (
      confirm(
        "are you sure you want to delete this routine- this action is permanent."
      )
    ) {
  ...
  return (
    <div id="routine-card">
      ...
      <p>
        <span className="beforeContent">Goal: </span>
        {routine.goal}
      </p>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          attachActivityToRoutine(routine.id, activityId, count, duration);
        }}
      >
        <label htmlFor="add-activity" className="addActivityLabel">
          ...
          ? routine.activities.map((activity, idx) => {
              return (
                <div key={idx} className="activity-card-onRoutine">
                  ...
                  <p>
                    <span className="beforeContent">Duration: </span>
                    {activity.duration} minutes
                  </p>
                  <p>
                    <span className="beforeContent">Count: </span>x
                    {activity.count}
                  </p>
                  {isEdit ? (
                    <EditActivity
      ...
      <Link to={`/edit/${routine.id}`}>
        <button className="myRoutinesButtons">edit routine</button>
      </Link>
      <button
        className="myRoutinesButtons"
        onClick={(e) => {
